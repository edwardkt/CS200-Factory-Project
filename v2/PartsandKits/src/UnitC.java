...
public class UnitC {
	...
	public UnitC(UnitB nlane1, UnitB nlane2, UnitB nlane3, UnitB nlane4, ArrayList<Nest> nests) {
		this.nests = nests;
		
		counter = 0;
		takePic = Toolkit.getDefaultToolkit().createImage("images/flash_dark.png");
		occupied = false;
		...
		partRobot = new PartRobot(250, 125, nests, kits);
	...
	public void bringToConveyor()
	{
		//kitRobot.giveNewPos(x,y);
		if(partRobot.kitStatus() == true)
			kits.get(0).giveNewPos(200,800);
		else
			kits.get(0).giveNewPos(0,420);
	}
	...
	public boolean kitStat()
	{	return partRobot.kitStatus();}
	...
	public void updateImage(Graphics2D g2){
		...
		if(!kits.isEmpty()){
			...
			if(kits.get(0).getX()==200 && kits.get(0).getY()==620)
				kits.remove(0);
			if(kits.get(0).getX()==0 && kits.get(0).getY()==420)
				kits.remove(0);
			...
			if(takingPicture)
			{
				...
				//g2.fillOval(188,360,70,70);
				g2.drawImage(takePic,120,300,250,250,null);
				counter++;
