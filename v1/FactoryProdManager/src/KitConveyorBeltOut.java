...
public class KitConveyorBeltOut{
	...
	public KitConveyorBeltOut(int newx, int newy) {
		x = newx;
		y = newy;
		dropOffPoint = new Point(x+width/2, y + height/2 );
	...
	public void UpdateImage(Graphics2D g2) {
		...
		if(laneOn)
		{
			...
			{
				lineList.remove(0);
				lineList.add(3, new Rectangle2D.Double( x+57, y, 5, 90 ));
			}
			if(lanecounter<100)
			{
				for(int i=0; i<4; i++)
